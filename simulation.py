...
def compute_EV(N, M, k, strategies, mu, p_s, neuron_types=[], kappa=-1.0, autapse=False):
...
def optimize_neuron_numerical(N, M, k, strategies, mu, p_s, lr, neuron_types=[], kappa=-1.0, bound=0.0, autapse=False):
    grad = np.zeros(M)
    eps = 1e-12
    for i in range(M):
        ...
        EV_plus = compute_EV(N, M, k, strategies, mu, p_s, neuron_types, kappa, autapse)
        ...
        EV_minus = compute_EV(N, M, k, strategies, mu, p_s, neuron_types, kappa, autapse)
...
def simulation(N, M, T, mu, lr, signal_distr, method, neuron_types=[], kappa=-1.0, bound=0.0, autapse=False):
...
def simulation_average(num_trials, N, M, T, mu, lr, signal_distr, method, neuron_types=[], kappa=-1.0, bound=0.0, autapse=False):
    runs = []
    neuron_strategies = []
    for trial in range(num_trials):
        encoding_qualities, strategies = simulation(N, M, T, mu, lr, signal_distr, method, neuron_types, kappa, bound, autapse)
